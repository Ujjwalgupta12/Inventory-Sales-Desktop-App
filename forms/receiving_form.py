from PySide6.QtWidgets import (
    QWidget, QLabel, QVBoxLayout, QHBoxLayout, QLineEdit, QPushButton, QComboBox, QMessageBox
)
...
class GoodsForm(QWidget):
    def __init__(self):
        ...
        layout.addWidget(QLabel("Goods Receiving Form"))
    ...
    def submit_form(self):
        ...
        """)
        cursor.execute("""
            INSERT INTO goods_receiving (product, supplier, quantity, unit, rate, total, tax, final_amount)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (product, supplier, qty, unit, rate, total, tax_val, final))
        conn.commit()
        conn.close()

        QMessageBox.information(self, "Success", "Goods entry saved successfully!")
        self.clear_fields()
